...
// 150902 나진수 주문하기 페이지 머무는 시간 카운트 하는 부분 추가 시간 지난 후 메인으로 돌아감 시작

$stocktime_ct_time = date('Y-m-d H:i:s', G5_SERVER_TIME-1200); // 주문하기 페이지 머무는 시간 카운팅
$sql_ct_selet_time = " select count(*) as cnt
                from {$g5['g5_shop_cart_table']}
                where od_id = '$tmp_cart_id'
                  and ct_status = '쇼핑'
                  and ct_select = '1'
                  and ct_select_time > '$stocktime_ct_time' ";
$row_ct_selet_time = sql_fetch($sql_ct_selet_time);

if (!$row_ct_selet_time['cnt']) {
    $sql_ct_selet_out = "update {$g5['g5_shop_cart_table']} set ct_select = '0' where ct_select='1' and od_id='$tmp_cart_id' and ct_status = '쇼핑' ";
    sql_query($sql_ct_selet_out);
    goto_url(G5_SHOP_URL);
}
// 150902 나진수 주문하기 페이지 머무는 시간 카운트 하는 부분 추가 시간 지난 후 메인으로 돌아감 끝
?>
<script language='javascript'> 
  window.setTimeout('window.location.reload()',1200000); //1초마다 리플리쉬 시킨다 1000이 1초가 된다. 
</script>
...
for ($i = 0; $row = mysql_fetch_array($result); $i++) {
    ...
    if (!$goods) {
        ...
        $goods = preg_replace("/\?|\'|\"|\||\,|\&|\;/", "", preg_replace("[<br/>]", "", $row['it_name']));
    ...
    <?php if ($default['de_tax_flag_use']) { ?>
                ...
                <div class="li_name_od">
                    <?php echo $image; ?>
                    <span><?php echo preg_replace("<br/>", "/\'/", $row['it_name']); ?></span>
                </div>
    <?php
    // 20150828 나진수 장바구니 형태 변경 orderinquiryview 형태로 변경됨
    $sql_option = " select ct_id, it_name, ct_option, ct_qty, ct_price,  ct_status, io_type, io_price
                            from {$g5['g5_shop_cart_table']}
                            where od_id = '$s_cart_id'
                              and it_id = '{$row['it_id']}' ";
    $res_option = sql_query($sql_option);

    for ($a = 0; $opt = sql_fetch_array($res_option); $a++) {
        if ($opt['io_type'])
        ...
        ?>
                    ...
                    <div class="li_prqty">
                        <span class="prqty_price li_prqty_sp"><span>판매가 </span><?php echo number_format($opt_price); ?>원</span>
                    ...
                    <div class="li_total">
                        <span class="total_price total_span"><span>주문금액 </span><strong><?php echo number_format($sell_price); ?>원</strong></span>
                    </div>
...
<div id="sod_frm">
    <form name="forderform" method="post" action="<?php echo $order_action_url; ?>" autocomplete="off">
            ...
            <div class="odf_tbl">
                <table>
                    <tbody>
                        ...
                        <!--150828 나진수 핸드폰 번호 입력란 최대 11개까지 변경 및 숫자만 입력가능하도록 변경 -->
                        <tr>
                            <th scope="row"><label for="od_hp">핸드폰</label></th>
                            <td><input type="tel" pattern="[0-9]*" name="od_hp" onkeyup="this.value = this.value.replace(/\D/, '')" placeholder="( - 없이 숫자만 입력)" value="<?php echo $member['mb_hp']; ?>" id="od_hp" style="line-height:normal;" required class="frm_input required" maxlength="11"></td>
                        </tr>
                        <tr>
                        ...
                        <tr style="display:none">
                            ...
                            <td>
                                ...
                                <input type="text" name="od_zip" value="<?php echo $member['mb_zip1'] . $member['mb_zip2']; ?>" id="od_zip" class="frm_input" size="10" maxlength="6">
                                ...
                                <input type="text" name="od_addr1" value="<?php echo $member['mb_addr1'] ?>" id="od_addr1" class="frm_input frm_address">
                ...
                <table>
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><input type="tel" name="od_b_hp" onkeyup="this.value = this.value.replace(/\D/, '')" placeholder="( - 없이 숫자만 입력)" value="<?php echo $member['mb_b_hp']; ?>" id="od_b_hp" style="line-height:normal;" required class="frm_input required" maxlength="11"></td>
                        </tr>
                        <tr>
                            <th scope="row">주소</th>
                            <td id="sod_frm_addr"><!-- 150811 나진수 구매자 주소 우편번호 / 기본주소 박스 수정 못하게 / 주소검색 되게 작동 소스 추가  onclick ="win_zip()" readonly="readonly"-->
                                <label for="od_b_zip" class="sound_only">우편번호<strong class="sound_only"> 필수</strong></label>
                                <input type="text" readonly="readonly" onclick="win_zip('forderform', 'od_b_zip', 'od_b_addr1', 'od_b_addr2', 'od_b_addr3', 'od_b_addr_jibeon');
                                        return false;" name="od_b_zip" id="od_b_zip" required class="frm_input required" size="10" maxlength="6">
                                ...
                                <input type="text" readonly="readonly" name="od_b_addr1" onclick="win_zip('forderform', 'od_b_zip', 'od_b_addr1', 'od_b_addr2', 'od_b_addr3', 'od_b_addr_jibeon');
                                        return false;" id="od_b_addr1" required class="frm_input frm_address required">
        ...
        if ($default['de_bank_use']) {
            $multi_settle++;
            echo '<li><input type="radio" id="od_settle_bank" name="od_settle_case" value="무통장" ' . $checked . '> <label for="od_settle_bank">무통장</label></li>' . PHP_EOL;
            $checked = '';
        }
...
<script>
    $(function () {
        ...
        // 배송지선택
        if ($('input:radio[name=ad_sel_addr]:checked').val() == 'same') {
            gumae2baesong(true);
        }
...
// 주문폼 필드체크
    function orderfield_check(f)
    {
        ...
        check_field(f.od_email, "주문하시는 분 이메일을 입력하십시오.");
        //check_field(f.od_addr1, "주소검색을 이용하여 주문하시는 분 주소를 입력하십시오.");
        //check_field(f.od_addr2, " 주문하시는 분의 상세주소를 입력하십시오.");
        //check_field(f.od_zip, "");

        //150828 나진수 주문하시는분 휴대전화번호 길이 변경 추가 시작
        if (f.od_hp.value.length < 10) {
            alert("주문하시는 분 핸드폰번호는 10~11자 이내로 입력하여 주십시오.");
            f.od_hp.focus();
            return false;
        }
        //150828 나진수 주문하시는분 휴대전화번호 길이 변경 추가 끝
        ...
        if (f.od_b_hp.value.length < 10) {
            alert("받으시는 분 핸드폰번호는 10~11자 이내로 입력하여 주십시오.");
            f.od_b_hp.focus();
            return false;
        }
