...
// 150903 나진수 주문후 이니시스 결제 데이터 입력 후 오는 쪽에 ct_select 값이 0일경우 1로 다시 넘김 시작
$sql_new_select = "update {$g5['g5_shop_cart_table']}
           set ct_select = '1', ct_select_chk='1'
         where od_id = '$tmp_cart_id'
           and ct_select = '0' and ct_select_chk='0' ";
sql_query($sql_new_select);
// 150903 나진수 주문후 이니시스 결제 데이터 입력 후 오는 쪽에 ct_select 값이 0일경우 1로 다시 넘김 끝
if (get_cart_count($tmp_cart_id) == 0)// 장바구니에 담기
    alert('결제가 완료되었습니다.\\n\\n 주문조회에서 확인하실 수 있습니다.', G5_SHOP_URL.'/orderinquiry.php');
...
// 150903 나진수 재고 입금 기준으로 변경으로 인한 에러부분 삭제
// 모듈 진입시 다른 사용자가 입금 하였을 경우 뜨는 경고창으로 인한 주문 date 미등록되어서 주석 처리됨
//if ($error != "")
//{
...
$od_id_sms = get_session('ss_order_id');
            ...
            $sms_contents = str_replace("{주문번호}", $od_id_sms, $sms_contents);
...
$od_b_email          = clean_xss_tags($od_b_email);
...
if($config['cf_sms_use'] && ($default['de_sms_use2'] || $default['de_sms_use3'])) {
    ...
    if($is_sms_send&&$od_settle_case == '가상계좌') {
        ...
        for($s=0; $s<count($sms_contents); $s++) {
            ...
            $sms_content = str_replace("{가상계좌번호}", $od_bank_account, $sms_content);
