...
$bn_slide_btn = '';
$bn_sl = ' class="bn_sl"';
//$first_img =''; 150815 영카트 버전 5.0.42 버전에서 내가 소스 자체 변형에서 영카트 버전으로 다시 변경함 
for ($i=0; $row=sql_fetch_array($result); $i++)
{
    /*if($i>=1) $first_img = 'swipe';       150815 나진수 영카트 5 패치 5.0.43  버전에서 내가 소스 자체 변형에서 영카트 버전으로 다시 변경함 
    if ($i==0) echo '<div id="main_bn" class="'.$first_img.'">'.PHP_EOL.'<ul class="slide-wrap bn_img">'.PHP_EOL;*/
    if ($i==0) echo '<div id="main_bn" class="swipe">'.PHP_EOL.'<ul class="slide-wrap bn_img">'.PHP_EOL; //150815 나진수 영카트 5 패치 5.0.43 버전으로 변경하여 이미지 하나일때 나타나게 함 
    //print_r2($row);
    // 테두리 있는지
