    ...
    for ($i=1, $max = count($item); $i<$max; $i++)
    {
        ...
        switch ($item[0])
        {
            ...
            case 'l':
                ...
                $qry = sql_query("select mb_id, mb_name, mb_hp from {$g5['member_table']} where mb_level='$mb_level' and mb_sms=1 and not (mb_hp='')");
                while ($row = sql_fetch_array($qry))
                {
                    $name = $row['mb_name'];
...
if ($result)
{
    ...
    if ($result) //SMS 서버에 접속했습니다.
    {
        ...
        foreach ($SMS->Result as $result)
        {
            ...
            //$log = @iconv('UTF-8', 'UTF-8//IGNORE', $log);// 150815  나진수 영카트 5 패치 5.0.43 버전  주석처리
            $log = @iconv('euc-kr', 'utf-8', $log); // 150815  나진수 영카트 5 패치 5.0.43 버전 (sms5 로그 기록 한글깨짐 오류 수정)
