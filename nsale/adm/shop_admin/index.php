...
function get_order_date_sum($date) {
    ...
    $info['received'] = (int) $row['received_price'];   // 150824 나진수 입금 그래프 추가
...
foreach ($arr_order as $val) {
    ...
    if ($val['received'] > 0)
        $h3 = intval(($max_height * $val['received']) / $max_y) + $offset;
    else
        $h3 = 0;
        ...
        ?>
            ...
            <ul id="sidx_graph_area">
                ...
                for ($i = 0; $i < count($x_val); $i++) {
                    ...
                    $received_title = date("n월 j일", strtotime($x_val[$i])) . ' 입금: ' . display_price($arr_order[$i]['received']);
                    $k = 10 - $i;
                    $li_bg = 'bg' . ($i % 2);
                    ?>
                    <li class="<?php echo $li_bg; ?>" style="z-index:<?php echo $k; ?>">
                        ...
                        <div class="graph received" title="<?php echo $received_title; ?>">
...
foreach ($x_val as $val) {
    ?>
            ...
            <div id="sidx_graph_legend">
                ...
                <span id="legend_received"></span> 입금 <!-- 150824 나진수 입금 그래프 추가-->
            </div>
        </div>
    </section>
...
//                .css({ left: left+"px", bottom: bottom+"px" })
    ...
    function graph_draw()
    {
        ...
        var g_h3 = new Array("<?php echo implode('", "', $h_val['received']); ?>"); 
