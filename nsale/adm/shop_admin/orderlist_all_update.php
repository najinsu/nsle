...
//150903 나진수 주문내역 검색시 뷰티상품 또는 일반 상품 ca_id값이 없을때 나타냄 시작 
if ($_POST['ca_id'] == '') {
    alert("뷰티상품 또는 일반상품 중 하나를 선택해주세요");
}
//150903 나진수 주문내역 검색시 뷰티상품 또는 일반 상품 ca_id값이 없을때 나타냄 끝

$sql_all = "select  sc.it_name, si.ca_id, so.* from {$g5['g5_shop_order_table']} so left join {$g5['g5_shop_cart_table']} sc on (so.od_id=sc.od_id) left join {$g5['g5_shop_item_table']} si on(sc.it_id=si.it_id ) where so.od_status = '{$_POST['od_status']}' and si.ca_id = '{$_POST['ca_id']}' group by so.od_id ";

$result_all = sql_query($sql_all);

for ($i = 0; $row_all = sql_fetch_array($result_all); $i++) {
    // 실제 번호를 넘김
    $od_id = $row_all['od_id'];

    $invoice = $row_all['od_invoice'];
    $invoice_time = $row_all['od_invoice_time'];
    $delivery_company = $row_all['od_delivery_company'];
    ...
    //echo $od_id . "<br>";
    $change_status = "";
    if ($_POST['od_status'] == '주문')
        $change_status = "입금";
    if ($_POST['od_status'] == '입금')
        $change_status = "준비";
    if ($_POST['od_status'] == '준비')
        $change_status = "배송";
    if ($_POST['od_status'] == '배송')
        $change_status = "완료";
    ...
    switch ($current_status) {
        ...
        case '준비' :
            ...
            // SMS
            if ($config['cf_sms_use'] == 'icode' && $default['de_sms_use5']) {
...
// SMS
if ($config['cf_sms_use'] == 'icode' && $sms_count) {
    $SMS->Send();
}
