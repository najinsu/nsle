...
if ($csv == 'xls') {
    ...
    $sql = " SELECT a.od_id, od_b_zip1, od_b_zip2, od_b_addr1, od_b_addr2, od_b_addr3, od_b_addr_jibeon, od_b_name,  od_b_hp, od_delivery_company, b.it_name, ct_qty, b.it_id, a.od_id, od_memo, od_invoice, b.ct_option, b.ct_send_cost, b.it_sc_type
               FROM {$g5['g5_shop_order_table']} a, {$g5['g5_shop_cart_table']} b
              where a.od_id = b.od_id ";
    if ($case == 1) // 출력기간
        $sql .= " and a.od_time between '$fr_date 00:00:00' and '$to_date 23:59:59' ";
    ...
    // Put Excel data
    $data = array('주문번호', '주문자명', '상품명', '옵션명', '수량', '배송자명', '배송지 전화', '우편번호', '배송지주소', '배송회사', '운송장번호');
    ...
    for ($i = 1; $row = sql_fetch_array($result); $i++) {
        ...
        $row = array_map('iconv_euckr', $row);
// 150811 나진수 앞에 뛰어쓰기 공백 없앰 우편번호만 문자형태로 변형 액셀은 공백이 있으면 문자형태로 변형됨
        $worksheet->write($i, 0, $row['od_id'].' ');
        $worksheet->write($i, 1, $row['od_b_name']);
        $worksheet->write($i, 2, $row['it_name']);
        $worksheet->write($i, 3, $row['ct_option']);
        $worksheet->write($i, 4, $row['ct_qty']);
        $worksheet->write($i, 5, $row['od_b_name']);
        $worksheet->write($i, 6, conv_telno($row['od_b_hp']));
        $worksheet->write($i, 7, $row['od_b_zip1'] . $row['od_b_zip2'].' ' );
        $worksheet->write($i, 8, print_address($row['od_b_addr1'], $row['od_b_addr2'], $row['od_b_addr3'], $row['od_b_addr_jibeon']));
        $worksheet->write($i, 9, $row['od_delivery_company']);
        $worksheet->write($i, 10, $row['od_invoice']);
        $worksheet->write($i, 11, $row['od_memo']);
    }
...
for ($i = 0; $row = sql_fetch_array($result); $i++) {
    ...
    // 지운아빠 2013-04-18
    if ($row1['od_name'] == $row1['od_b_name'] && $row1['od_addr'] == $row1['od_b_addr'] && $row1['od_tel'] == $row1['od_b_tel'] && $row1['od_hp'] == $row1['od_b_hp'] && $row1['od_hp'] != "&nbsp;")
        $samesamesame = 1;
    else
        $samesamesame = '';
    ...
    if ($od_memo)
        $od_memo = "<p><strong>비고</strong> $od_memo</p>";
    if ($od_shop_memo)
        $od_shop_memo = "<p><strong>상점메모</strong> $od_shop_memo</p>";
