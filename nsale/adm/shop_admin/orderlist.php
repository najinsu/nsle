...
if ($search != "") {
    ...
    // 150828 나진수 sql 쿼리의 변경으로 인한 so. 앞에 추가
if ($od_status) {
    switch ($od_status) {
        case '전체취소':
            $where[] = " so.od_status = '취소' ";
            break;
        case '부분취소':
            $where[] = " so.od_status IN('주문', '입금', '준비', '배송', '완료') and od_cancel_price > 0 ";
            break;
        default:
            $where[] = " so.od_status = '$od_status' ";
            break;
    }
    ...
    // 150828 나진수 sql 쿼리의 변경으로 인한 so. 앞에 추가
if ($od_settle_case) {
    $where[] = " so.od_settle_case = '$od_settle_case' ";
}
...
if ($od_cancel_price) {
    $where[] = " so.od_cancel_price != 0 ";
}

if ($od_refund_price) {
    $where[] = " so.od_refund_price != 0 ";
}

if ($od_receipt_point) {
    $where[] = " so.od_receipt_point != 0 ";
}

if ($od_coupon) {
    $where[] = " so.od_coupon != 0 ";
}

if ($od_escrow) {
    $where[] = " so.od_escrow = 1 ";
}
    //150828 나진수 카테고리별 분류 검색으로 인한 추가부분
if ($ca_id) {
    $where[] = "  si.ca_id = '$ca_id' ";
}
if ($fr_date && $to_date) {
    $where[] = " so.od_time between '$fr_date 00:00:00' and '$to_date 23:59:59' ";
}
...
if ($sort2 == "")
    $sort2 = "desc";
// 150828 나진수 중복되는 아이디값 제거
$overlap_id = " group by sc.od_id";

// 150828 나진수 검색으로 인한 컬럼 변경
$sql_common = " from {$g5['g5_shop_order_table']} so left join {$g5['g5_shop_cart_table']} sc on (so.od_id=sc.od_id) left join {$g5['g5_shop_item_table']} si on(sc.it_id=si.it_id) $sql_search ";
// 150828 나진수 중복되는 아이디값 제거 -> 전체 게시글 카운팅
$sql = " select count(distinct so.od_id) as cnt " . $sql_common;
...
// 150828 나진수 검색으로 인한 컬럼 변경 및 $overlap_id 추가
$sql = " select  sc.it_name, si.ca_id, so.*
           $sql_common
               $overlap_id
           order by $sort1 $sort2 
           limit $from_record, $rows ";
$result = sql_query($sql);
//150828 나진수 분류검색으로 인한 추가 ca_id=$ca_id&amp;
$qstr1 = "od_status=" . urlencode($od_status) . "&amp;od_settle_case=" . urlencode($od_settle_case) . "&amp;od_misu=$od_misu&amp;od_cancel_price=$od_cancel_price&amp;od_refund_price=$od_refund_price&amp;od_receipt_point=$od_receipt_point&amp;od_coupon=$od_coupon&amp;fr_date=$fr_date&amp;to_date=$to_date&amp;ca_id=$ca_id&amp;sel_field=$sel_field&amp;search=$search&amp;save_search=$search";
...
<!-- 150828 나진수 나누어진 검색이 아닌 통합 검색으로 변경 및 상품 검색 추가-->
<form name="frmorderlist" class="local_sch02 local_sch">
    <input type="hidden" name="doc" value="<?php echo $doc; ?>">
    <input type="hidden" name="sort1" value="<?php echo $sort1; ?>">
    <input type="hidden" name="sort2" value="<?php echo $sort2; ?>">
    <input type="hidden" name="save_search" value="<?php echo $search; ?>">
    <!-- 150831 나진수 page 카운팅 방지를 위한 소스 삭제 -->
    <div style="padding-bottom:10px;">
        <label for="sel_field" class="sound_only">검색대상</label>
        <select name="sel_field" id="sel_field" >
            <option value="so.od_id" <?php echo get_selected($sel_field, 'so.od_id'); ?>>주문번호</option>
            <option value="so.mb_id" <?php echo get_selected($sel_field, 'so.mb_id'); ?>>회원 ID</option>
            ...
            <option value="so.od_deposit_name" <?php echo get_selected($sel_field, 'so.od_deposit_name'); ?>>입금자</option>
            <option value="od_invoice" <?php echo get_selected($sel_field, 'od_invoice'); ?>>운송장번호</option>
            <option value="sc.it_name" <?php echo get_selected($sel_field, 'sc.it_name'); ?>>상품명</option>
        </select>
    ...
    <!-- 150828 나진수 카테고리 분류 검색 추가-->
    <div>
        <strong>분류</strong>
        <input type="radio" name="ca_id" value="" id="ca_id"        <?php echo get_checked($ca_id, ''); ?>>
        <label for="ca_id">전체</label>
        <input type="radio" name="ca_id" value="10" id="ca_id10"   <?php echo get_checked($ca_id, '10'); ?>>
        <label for="ca_id10">뷰티상품</label>
        <input type="radio" name="ca_id" value="20" id="ca_id20" <?php echo get_checked($ca_id, '20'); ?>>
        <label for="ca_id20">일반상품</label>
    </div>
    <div>
        <strong>주문상태</strong>
...
<!-- 150828 나진수 일괄처리 기능 추가
        150831 나진수 입금 단계에서만 일괄 처리할수있도록 함
-->

<?php if($od_status == '입금'){?>
<form name="forderlist_all" onsubmit="return forderlist_submit_all(this);" method="post" autocomplete="off">
    <input type="hidden" name="od_status" value="<?php echo $od_status; ?>">
    <input type="hidden" name="ca_id" value="<?php echo $ca_id; ?>">
    <div style="float:right;margin:0px 20px 10px; color:#ff4347; font-weight: bold;">* 일괄처리시 꼭 체크상태를 확인 및 검색 버튼을 다시 클릭 후 진행하시기 바랍니다.<input type="submit" style="padding:3px 5px;margin-left:10px;" value="일괄처리"  class="btn_submit" onclick="document.pressed = this.value"></div>
</form>
<?php } ?>
...
for ($i = 0; $row = sql_fetch_array($result); $i++) {
    ...
    $bg = 'bg' . ($i % 2);
    $td_color = 0;
    // 150901 나진수 취소 기준이 $row['od_cancel_price'] > 0  가상결제 후 주문시 취소는 주문취소금액이 0이라 못잡는걸 방지하기 위해 변경
    if ($row['od_status'] =='취소' ) {
        $bg .= 'cancel';
        $td_color = 1;
    }
    
    /* 150903 나진수 에러시 노란색으로 발생 시작
    if (($row['od_settle_case'] =='신용카드' && !preg_match("/(_CARD)/", $row['od_tno']))|| ($row['od_settle_case'] =='신용카드' && !preg_match("/(_ISP)/", $row['od_tno']))||($row['od_settle_case'] =='휴대폰' && !preg_match("/(HPP)/", $row['od_tno']))||($row['od_settle_case'] =='가상계좌' && !preg_match("/(VBNK)/", $row['od_tno']))){
        $bg = 'error';
        $td_color = 1;
    }
     150903 나진수 에러시 노란색으로 발생 끝*/
    
    //150824 나진수 상품 갯수 카운팅 
    $sql_item = " select *, SUM(ct_qty) as qty
                                            from {$g5['g5_shop_cart_table']}
                                            where  od_id = '{$row['od_id']}' ";
    $sum = sql_fetch($sql_item);
    ?>
    ...
    <?php echo $od_paytype; ?>
                        ...
                        <td headers="th_odrid" class="td_od_id">
                            <?php if ($row['mb_id']) { ?>
                        ...
                        <td rowspan="2"><?php echo $sum['it_name']; ?></td>
    ...
    <?php if ($od_status == '준비') { ?>
                        ...
                        <td headers="th_odrertel" class="td_hp"><?php echo get_text($row['od_hp']); ?></td>
                        <td headers="th_odrertel" class="td_hp"><?php echo get_text($row['od_b_hp']); ?></td>
                    </tr>
                    <tr class="<?php echo $bg; ?>">
    ...
    <?php if ($od_status == '준비') { ?>
                        ...
                        <td headers="th_odrcnt"><?php echo $sum['qty']; ?>개</td>
                        <td class="td_addr" ><?php echo "(" . $row['od_b_zip1'] . $row['od_b_zip2'] . ") " . $row['od_b_addr1'] . $row['od_b_addr2']; ?></td>
                    </tr>
    <?php
    //150903 나진수 수량 갯수 수정
    $tot_itemcount += $sum['qty'];
...
?>
            ...
            <tfoot>
                <tr class="orderlist">
                    ...
                    <td><?php echo number_format($tot_itemcount); ?>개</td>
                    <th scope="row">합 계</th>
...
<script>
    /* 150828 나진수 일괄처리 스크립트 추가 */
    function forderlist_submit_all(f) {
        if (document.pressed == "일괄처리") {
            if (confirm("일괄처리시 신중하게 해주세요.")) {
                f.action = "./orderlist_all_update.php";
                return true;
            }
            return false;
        }
    ...
    function forderlist_submit(f)
    {
        ...
        for (var i = 0; i < chk.length; i++)
        {
            if (chk[i].checked)
            {
                ...
                {
                    case "입금" :
                        if (!(current_status == "주문" && current_settle_case == "가상계좌")) {
