...
<div class="tbl_head01 tbl_wrap">
    ...
    <tbody>
    <?php
    $sql = " select mb_id, mb_name,  mb_email, mb_datetime $sql_common $sql_where order by mb_id ";
    ...
    while ($row=sql_fetch_array($result)) {
        $i++;
        $ma_list .= $cr . $row['mb_email'] . "||" . $row['mb_id'] . "||" . get_text($row['mb_name']) . "||" . $row['mb_datetime'];
        $cr = "\n";
