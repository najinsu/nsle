...
if ($token && get_session("ss_token") == $token) {
   ...
   //  alert_close("토큰 에러"); 150815 나진수 영카트 5 패치 5.0.44 하면서 주석처리
    set_session("ss_token", "");    //150815 나진수 영카트 5 패치 5.0.44 xss 취약점 수정 
    alert("토큰 에러", G5_SHOP_URL); //150815 나진수 영카트 5 패치 5.0.44 다른 회원 xss 취약점 수정 
}
if($is_member){
...
// 주문상품의 상태가 주문인지 체크
$sql = " select SUM(IF(ct_status = '주문', 1, 0)||(ct_status = '입금',1, 0)) as od_count2,
                COUNT(*) as od_count1
...
// 주문 취소
//$cancel_memo = addslashes($cancel_memo);  //150815 나진수 영카트 5 패치 5.0.44 xss 취약점 주석처리
$cancel_memo = addslashes(strip_tags($cancel_memo)); //150815 나진수 영카트 5 패치 5.0.44 xss 취약점 

    //150901 나진수 주문 취소시 입금이 0이상 금액 기준에서 배송비 0원에서 취소되어도 배송비 합산하여 계산하도록 합
    if($od['od_receipt_price']>0){
        $cancel_price = $od['od_cart_price']+$od['od_send_cost']+$od['od_send_cost2'];
    }else{
        $cancel_price=0;    
    }

//150901 나진수 주문 취소시 배송비 0원에서 취소되어도 배송비 합산하여 계산하도록 합  od_send_cost = '0'->od_send_cost = '{$od['od_send_cost']}'   od_send_cost2 = '0',->  od_send_cost2 = '{$od['od_send_cost2']}',
$sql = " update {$g5['g5_shop_order_table']}
            set od_send_cost = '{$od['od_send_cost']}',
