...
class item_list {
    ...
    protected $view_it_cust_price = true;  // 소비자가
...
function get_it_stock_qty($it_id) {
    ...
    //150826 나진수 ct_status in ('주문' '입금', '준비') " 에서 입금상태로 변경
    // 재고에서 빼지 않았고 주문인것만
...
function it_img_upload($srcfile, $filename, $dir) {
    ...
    $filename = preg_replace_callback(
            "/[가-힣]+/", create_function('$matches', 'return base64_encode($matches[0]);'), $filename);
...
function get_item_options($it_id, $subject) {
    ...
    $sql = " select a.*, b.it_price from {$g5['g5_shop_item_option_table']} a left join {$g5['g5_shop_item_table']} b on a.it_id= b.it_id where a.it_id = '$it_id' order by a.io_no asc";// 150812 나진수 상품 정렬 순서 변경( 소스 추가) order by a.io_no asc
    $result = sql_query($sql);
    if (!mysql_num_rows($result))
        return '';
    ...
    if ($subj_count > 1) {
        ...
        for ($i = 0; $row = sql_fetch_array($result); $i++) {
            ...
            if ($row['io_price'] >= 0){
                $io_price_new_value = $row['it_price']+$row['io_price'];
                $price = '&nbsp;&nbsp;| ' . number_format($io_price_new_value) . '원';
            }
            else{
                $io_price_new_value_minus = $row['it_price']+$row['io_price'];
                $price = '&nbsp;&nbsp;| ' . number_format($io_price_new_value_minus) . '원';
            }
...
function print_item_options_cart_web($it_id, $cart_id) {
    global $g5;

    $sql = " select ct_option, ct_qty, io_price, io_type, ct_price, io_price
                from {$g5['g5_shop_cart_table']} where it_id = '$it_id' and od_id = '$cart_id' order by io_type asc, ct_id asc ";
    $result = sql_query($sql);
...
if($row['io_type']==0){
		...
		$it_total_pr=$row['ct_qty']*$price;
        if ($i == 0)
            $str .= '<ul id="cart_it_info">' . PHP_EOL;
        
         $mod_options = '<div class="sod_option_btn"><button type="button" class="mod_options">변경</button></div>';//변경
        $price_plus = '';
        if ($row['io_price'] >= 0)
            $price_plus = '+';
        $str .= '<li';
		if($row['io_type']==0){
		$str .= 'style = "color:#7c7c7c; "';	
		}
        $str .='><div class="cart_it_name">'.$row['ct_option'].' </div><div class="cart_it_price"> '.number_format($price).'</div><input type="text"   class="cart_input"  onfocus="this.blur();" readonly="readonly" value="'.$row['ct_qty'].'" >'.$mod_options.'<div class="cart_it_box_total">'.number_format($it_total_pr).'</div></li>'.PHP_EOL;
    }
...
// sns 공유하기
function get_sns_share_link($sns, $url, $title, $img, $it_img) {
    global $config;
    ...
    switch ($sns) {
        ...
        case 'kakaotalk':
            if ($config['cf_kakao_js_apikey'])
                $str = '<a href="javascript:kakaolink_send(\'' . str_replace('+', ' ', urlencode($title)) . '\', \'' . urlencode($url) . '\', \'' . $it_img . '\');" class="share-kakaotalk"><img src="' . $img . '" alt="카카오톡 링크보내기"></a>';
            break;
    }
...
function get_order_info($od_id) {
    ...
    if ($od['od_tax_flag']) {
        $tot_tax_mny = ( $tax_mny + $send_cost + $od['od_send_cost2'] ) - ( $od_coupon + $od_send_coupon + $od['od_receipt_point'] );
        if ($tot_tax_mny < 0) {
            $free_mny += $tot_tax_mny;
            $tot_tax_mny = 0;
        }
    } else {
        $tot_tax_mny = ( $tax_mny + $free_mny + $send_cost + $od['od_send_cost2'] ) - ( $od_coupon + $od_send_coupon + $od['od_receipt_point'] );
        $free_mny = 0;
    }
    ...
    // 미수금액
    // 150901 나진수 주문취소시 배송비 0원으로 되고 주문 취소가 아닐시 미수금은 2500원 합산하여 계산 시작
    if($cancel_price>0){
    $od_misu = ( $cart_price  ) - ( $cart_coupon + $od_coupon + $od_send_coupon ) - ( $od['od_receipt_price'] + $od['od_receipt_point'] - $od['od_refund_price'] );
    }else{
        $od_misu = ( $cart_price + $send_cost + $od['od_send_cost2'] ) - ( $cart_coupon + $od_coupon + $od_send_coupon ) - ( $od['od_receipt_price'] + $od['od_receipt_point'] - $od['od_refund_price'] );
    }
    // 150901 나진수 주문취소시 입금 금액이 배송비 0원으로 되고 주문 취소가 아닐시 미수금은 2500원 합산하여 계산 끝
    // 장바구니상품금액
    $od_cart_price = $cart_price + $cancel_price;
    if($od['od_receipt_price']>0){
    $total_can_sc=$cancel_price+$send_cost;    
    }else{
        $total_can_sc=0;
    }
    ...
    $info['od_cancel_price'] = $total_can_sc;    //150901 나진수 주문취소시 배송비 합산
...
function get_sendcost($cart_id, $selected = 1) {
    ...
    $total_price = 0;
    $total_send_cost = 0;
    // 150901 나진수 주문취소시 배송비 합산 (,'취소') 추가
    $sql = " select distinct it_id
                from {$g5['g5_shop_cart_table']}
                where od_id = '$cart_id'
                  and ct_send_cost = '0'
                  and ct_status IN ( '쇼핑', '주문', '입금', '준비', '배송', '완료','취소', '반품'  )
