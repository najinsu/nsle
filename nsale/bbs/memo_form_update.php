...
for ($i=0; $i<count($recv_list); $i++) {
    $row = sql_fetch(" select mb_id, mb_name, mb_open, mb_leave_date, mb_intercept_date from {$g5['member_table']} where mb_id = '{$recv_list[$i]}' ");
    if ($row) {
...
for ($i=0; $i<count($member_list['id']); $i++) {
    ...
    $recv_mb_nick = get_text($member_list['name'][$i]);
