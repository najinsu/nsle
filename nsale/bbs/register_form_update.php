...
if ($w == '' || $w == 'u') {
    ...
    //if ($msg = empty_mb_name($mb_id))       alert($msg, "", true, true); // 150815 나진수 영카트 5 패치 5.0.43 버전 전 소스 주석 처리
    if ($msg = empty_mb_name($mb_name))       alert($msg, "", true, true);//150815 나진수 영카트 5 패치 5.0.43 버전 mb_id 기준이 아닌 name기준으로 설정
