...
if ($w == '' || $w == 'r') {
    ...
    if ($member['mb_id']) {
        $mb_id = $member['mb_id'];
        $wr_name = addslashes(clean_xss_tags($board['bo_use_name'] ? $member['mb_name'] : $member['mb_id']));
...
for ($i=0; $i<count($_FILES['bf_file']['name']); $i++) {
    ...
    if (is_uploaded_file($tmp_file)) {
        ...
        // 올라간 파일의 퍼미션을 변경합니다.
        chmod($dest_file, G5_ALL_PERMISSION);
        
//        $__host = "52.68.171.168"; //접속할 서버의 ip나 도메인 
//        $__port = "21"; //접속할 서버의 포트. 보통은 21을 사용할것이다.
//        $__id = "root"; //접속할 서버의 id
//        $__pw = "wow2014@969802192W"; //접속할 서버의 password 
//        $__ftp_path = "/var/www/html/img/".$bo_table."/".$upload[$i]['file']; // 접속한 서버에 업로드되어 새로 생길 파일
//        
//        $fc = ftp_connect($__host, $__port);
//        $login_result = ftp_login($fc, $__id, $__pw);
//        
//        // 접속 되었는지 확인한다
//        if ((!$fc) || (!$login_result)) {
//            $sql = "insert into test_pay(test) values('FTP connection has failed !')";
//            sql_query($sql);
//        }
//
//        $sql = "insert into test_pay(test) values('Current directory: " . ftp_pwd($fc) . "\n')";
//        sql_query($sql);
//        
//        ftp_pasv($fc, true);
        
//        echo "Current directory: " . ftp_pwd($fc) . "\n";

        //파일 이동
//        if(ftp_chdir($fc, "/var/www/html/img"))
//        {
//            $sql = "insert into test_pay(test) values('ftp_chdir ok')";
//            sql_query($sql);
//        }
//        else
//        {
//            $sql = "insert into test_pay(test) values('ftp_chdir error')";
//            sql_query($sql);
//        }
//        $sql = "insert into test_pay(test) values('Current directory: " . ftp_pwd($fc) . "\n')";
//        sql_query($sql);
...
//        ftp_chdir($fc, "img");
        ...
        //폴더 생성
//        if(ftp_mkdir($fc, $bo_table))
//        {
//            ftp_chmod($fc, G5_DIR_PERMISSION, $bo_table);
//        }
//        else
//        {
//            
...
//        if(is_dir("/var/www/html/img/".$bo_table))
//        {
//            
//            $sql = "insert into test_pay(test) values('file true')";
//            sql_query($sql);
//        }
//        else
//        {
//            $sql = "insert into test_pay(test) values('file false')";
//            sql_query($sql);
//        }
        
        
        //ftp_put의 4번째 파라미터로 FTP_BINARY 또는 FTP_ASCII 를 선택할수 있다.
//        if (ftp_alloc($fc, filesize($dest_file), $result)) {
//            if(!ftp_put($fc, $__ftp_path, $dest_file, FTP_BINARY))
//            {
//                $sql = "insert into test_pay(test) values('ftp_path = $__ftp_path, dest_file = $dest_file')";
//                sql_query($sql);
//            }
//        }
//        else
//        {
//            $sql = "insert into test_pay(test) values('result = $result')";
//            sql_query($sql);
//        }
